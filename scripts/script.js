...
window.addEventListener("load", () => {
  ...
  const categoryFiltersContainer = document.querySelector(".skills-category-filters");
  ...
  skillCategories.forEach((category) => {
    ...
    if (title) {
      ...
      const iconClass = originalIcon ? originalIcon.className : "fa-solid fa-tag";
  ...
  function filterSkills() {
    ...
    skillCategories.forEach((category) => {
      const categoryTitle = category.querySelector(".skill-category-title").textContent.trim();
      const skillTags = category.querySelectorAll(".skill-tag");
      let categoryHasVisibleSkills = false;

      // Check if category matches active filter
      const categoryMatches = activeCategory === "all" || categoryTitle === activeCategory;

      skillTags.forEach((tag) => {
        const skillText = tag.textContent.toLowerCase();
        const matchesSearch = searchTerm === "" || skillText.includes(searchTerm);
